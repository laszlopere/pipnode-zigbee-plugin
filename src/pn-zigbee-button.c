 ...
 * fallback button label is whatever follows "zigbee2mqtt/".  Pinned to
...
/* fa-dot-circle U+F192 -- a button face (a ring with a centred dot), the
 * natural glyph for a single wireless push button.  Magenta body,
 * matching the rest of the Zigbee palette (#PnZigbeeSource /
 * #PnZigbeeRemote); a distinct glyph keeps it apart from the Remote's
 * pointing-finger on the worksheet. */
#define PN_ZIGBEE_BUTTON_ICON "\xef\x86\x92"

struct _PnZigbeeButton
{
    PnNode parent_instance;

    /* Optional filters.  An empty / NULL value is a wildcard (match
     * any), so a freshly-dropped node forwards every button press.  Set
     * either to narrow the node to one specific button and/or one
     * specific gesture. */
    gchar *friendly_name;
    gchar *action;
};

G_DEFINE_TYPE (PnZigbeeButton, pn_zigbee_button, PN_TYPE_NODE)
 ...
 *  strings -- availability, etc. -- which are never button events). */
static JsonObject *
...
/** Best human-readable name for the button behind @message.
 *
 *  Preference order:
 ...
 *       device is the IEEE address (e.g. 0xf044d3fffef9779d).
...
/** Does @message's button match the configured friendly-name filter?
 ...
 *  that is not a button press; for the survivors it stamps the
 ...
 *  receives. */
static void
pn_zigbee_button_receive (
        PnNode    *node,
        PnMessage *message)
{
    PnZigbeeButton *self = PN_ZIGBEE_BUTTON (node);
    JsonObject     *payload;
    const gchar    *action;
    const gchar    *name;
    gchar          *output;
    ...
    /* A real press carries a non-empty string `action` (SNZB-01P emits
     * "single" / "double" / "long").  The idle / state-only publishes
     * Z2M interleaves (and the "released" empty-action frames some
     * converters emit) have none, so they are filtered out here. */
     ...
     * default.  A configured value narrows the node to one button
     * and/or one gesture. */
    ...
    name = resolve_button_name (message, payload);
     ...
     * `data.payload` (action, battery, linkquality, injected device
     * block) are left intact for nodes that want the detail. */
    pn_message_set_boolean (message, "success", TRUE);
    pn_message_set_double  (message, "value",   1.0);

    output = g_strdup_printf ("Button %s pressed (action: %s)",
                              name, action);
...
static void
pn_zigbee_button_get_property (
        GObject    *object,
        guint       prop_id,
        GValue     *value,
        GParamSpec *pspec)
{
    PnZigbeeButton *self = PN_ZIGBEE_BUTTON (object);
...
static void
pn_zigbee_button_finalize (GObject *object)
{
    PnZigbeeButton *self = PN_ZIGBEE_BUTTON (object);
    ...
    G_OBJECT_CLASS (pn_zigbee_button_parent_class)->finalize (object);
}

static void
pn_zigbee_button_class_init (PnZigbeeButtonClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);
    PnNodeClass  *node_class   = PN_NODE_CLASS (klass);

    object_class->get_property = pn_zigbee_button_get_property;
    object_class->set_property = pn_zigbee_button_set_property;
    object_class->finalize     = pn_zigbee_button_finalize;

    node_class->receive = pn_zigbee_button_receive;

    node_class->palette_icon = PN_ZIGBEE_BUTTON_ICON;
    node_class->class_name   = "Zigbee Button";
    node_class->icon         = PN_ZIGBEE_BUTTON_ICON;
    ...
    props[PROP_FRIENDLY_NAME] = g_param_spec_string (
            ...
            "Optional button filter.  When set, only presses from this "
            "button pass; the value is matched against the injected "
            "data.payload.device.friendlyName (when an upstream Zigbee "
            "Source injects device info) or, failing that, the "
            "friendly-name tail of the envelope topic "
            "(zigbee2mqtt/<tail>, the bare device id for an unrenamed "
            "device).  Empty (default) matches every button.",
            NULL,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    props[PROP_ACTION] = g_param_spec_string (
            "action", "Action",
            "Optional gesture filter.  When set, only presses whose "
            "data.payload.action equals this exact value pass (e.g. "
            "\"single\", \"double\", \"long\").  Empty (default) matches "
            "every action.",
            NULL,
            G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
static void
pn_zigbee_button_init (PnZigbeeButton *self)
{
    ...
    pn_node_set_icon       (node, PN_ZIGBEE_BUTTON_ICON);
...
PnZigbeeButton *
pn_zigbee_button_new (void)
{
    return g_object_new (PN_TYPE_ZIGBEE_BUTTON, NULL);
}
